...
export default class BlurPostFX extends Phaser.Renderer.WebGL.Pipelines.PostFXPipeline {
    ...
    onPreRender() {
        const radius = 2.0; // Adjust the radius of the blur effect as desired
        const direction = { x: 1.0, y: 1.0 }; // Adjust the direction of the blur effect (1.0, 0.0) for x-axis blur, (0.0, 1.0) for y-axis blur

        this.set1f('uResolution', this.renderer.width);
        this.set1f('radius', radius);
        this.set2f('dir', direction.x, direction.y);
    }
}
